...
//                               <p className="text-sm text-gray-500 mt-1">{reel.views}</p>
//                             </div>
...
/////////////////////////FOR REEEELS SECTION//////////////////////
  ...
  return(
    <div>
       <div className={`flex flex-col sm:flex-wrap sm:flex-row justify-start gap-3 w-full `}>
          {/* <VideoCard  menucon={menucon} videos={videos} searchdata={searchdata} /> */}
            {(searchdata.length > 0 ? searchdata:firstThree).map((video , index) => {
            const thumbnail = `https://img.youtube.com/vi/${video.Id}/hqdefault.jpg`;
                return(
                    <div key={video.Id || index} onClick={() => window.open(`https://www.youtube.com/watch?v=${video.Id}`, "_blank")}
                        onMouseEnter={() => setHovered(video.Id)}
                        onMouseLeave={() => setHovered(null)}
                    className={`cursor-pointer h-max md:w-[32.2%] line-clamp-1     sm:w-[48.9%] w-[100%] pb-3 xl:pb-1 ${menucon===true? 'lg:w-[32.3%] xl:w-[32.5%] ': 'xl:w-[32.6%] lg:w-[32.43%] '}`}
                    >
